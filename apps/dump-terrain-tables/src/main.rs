...
fn make_wireframe_list(max_level: usize) {
    ...
    println!("// cargo run -p dump-terrain-tables -- --dump-wireframe-list");
    for subdivisions in 0..=max_level {
        ...
        for &winding in &PatchWinding::all_windings() {
            ...
            println!(
                "static WIREFRAME_INDICES{}_{}: [u32; {}] = {:?};",
                subdivisions,
                winding.const_name(),
                indices.len(),
                indices
...
fn make_triangle_strips(max_level: usize) {
    ...
    for subdivisions in 0..=max_level {
        ...
        for &winding in &PatchWinding::all_windings() {
            ...
            println!(
                "static TRI_STRIP_INDICES{}_{}: [u32; {}] = {:?};",
                subdivisions,
                winding.const_name(),
                indices.len(),
                indices
    ...
    for subdivisions in 0..=max_level {
        ...
        for &winding in &PatchWinding::all_windings() {
            func += &format!(
                "                PatchWinding::{} => &TRI_STRIP_INDICES{}_{},\n",
                winding.enum_name(),
                subdivisions,
                winding.const_name()
            );
